...
describe('capability artifact', () => {
  ...
  it('normalizes model prose into a deterministic cross-tenant checkpoint', () => {
    const capability = compileCapability({
      runId: 'discovery_test',
      entryUrl: 'http://127.0.0.1:4310/tenant/harbor',
      successText: 'The requested savings balance was extracted successfully.',
      actions: artifact().steps.map((action) => ({ action }))
    });
    expect(capability.success.checkpoint.locators).toEqual([
      { strategy: 'text', text: 'DEPOSIT ACCOUNTS', exact: true },
